...
import { useArrayObjectInput, useObjectInput } from "./hooks/useInput";
...
export default function TestUseArrayObjectInput() {
  ...
  const [
    ...
    onRemoveUsers,
    onUpdateUsers,
  ] = useArrayObjectInput(initialUser);
  const [user, setUser, onChangeUser, resetUser] = useObjectInput({
    name: "",
    email: "",
  });
  ...
  return (
    <>
      ...
      <ul>
        {users.map((oldUser) => (
          <li key={oldUser.id}>
            {oldUser.id} - {oldUser.name} ({oldUser.email}) &nbsp;
            <button onClick={() => handleRemoveUsers(oldUser.id)}>
              삭제
            </button>{" "}
            &nbsp;
            {/*<button onClick={() => onRemoveUsers("id", oldUser.id)}>삭제</button>*/}
            <button onClick={() => handleExchange("id", oldUser.id)}>
              변경
            </button>
          </li>
        ))}
