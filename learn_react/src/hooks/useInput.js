...
export function useArrayObjectInput(initialState) {
    ...
    const onRemove = (paramName, paramValue) => {
        setValue(value.filter((objectValue) => objectValue[paramName] !== paramValue));
    }

    const onUpdate = (keyName, KeyValue, paramName, paramValue) => {
        setValue(value.map((objectValue) => objectValue[keyName] === KeyValue ? {...objectValue, [paramName]: paramValue} : objectValue));
    }
